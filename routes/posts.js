...
router.get("/", async(req,res)=>{ // 라우터는 위에서 아래로 읽음? 
    try{
        const posts = await Post.find().sort("-createdAt"); // 1. 디비에 저장돼있는 데이터를 가져와서 작성 날짜 기준 내림차순해서 변수에 할당
...
// 게시글 상세조회
router.get("/:_postId", async(req,res)=>{ // 라우터는 위에서 아래로 읽어 진행
    try{
        //url뒤에 id값을 파라미터로 받아옴 / 주소 뒤에 어떤 데이터를 담아주는 걸 파라미터 
...
router.put("/:_postId", async(req,res)=>{
    try{// 파라미터에서 postId 값을 같은 변수 이름으로 구조분해 할당으로 갖고 온다
        const { _postId } = req.params
        ...
        if(posts.password == password){
            // 디비에 title과 content 값을 update 할 수 있게 함
        ...
        console.log(error)
        res.status(400).send({msg : "게시글 수정 실패"})
    }
})
...
router.delete("/:_postId", async(req,res)=>{
    try{// 파라미터에서 postId 값을 같은 변수 이름으로 구조분해 할당으로 갖고 온다
        const { _postId } = req.params
        // 파라미터에서 password 값을 같은 변수 이름으로 구조분해 할당으로 갖고 온다
        const {password} = req.body
        // 디비에 저장된 _id값을 찾아와 posts라는 변수에 할당
        const posts = await Post.findOne({_id:_postId})
        // 만약 디비에 저장된 패스워드와 내가 입력한 패스워드가 일치하면
        if(posts.password == password){
            // 디비에 title과 content 값을 삭제할 수 있게 함
